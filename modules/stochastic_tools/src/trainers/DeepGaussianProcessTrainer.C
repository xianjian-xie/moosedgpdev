...
#include "DeepGaussianProcessTrainer.h"
#include "Sampler.h"
...
registerMooseObject("StochasticToolsApp", DeepGaussianProcessTrainer);

InputParameters
DeepGaussianProcessTrainer::validParams()
{
...
DeepGaussianProcessTrainer::DeepGaussianProcessTrainer(const InputParameters & parameters)
  : SurrogateTrainer(parameters),
    ...
    _optimization_opts(StochasticTools::DeepGaussianProcess::DGPOptimizerOptions(
    ...
    _sampler_row(getSamplerData())
{
  ...
  _dgp.initialize(getCovarianceFunctionByName(parameters.get<UserObjectName>("covariance_function")),
                 tune_parameters,
                 lower_bounds,
                 upper_bounds);

  _n_outputs = _dgp.getCovarFunction().numOutputs();
}

void
DeepGaussianProcessTrainer::preTrain()
{
...
void
DeepGaussianProcessTrainer::train()
{
...
void
DeepGaussianProcessTrainer::postTrain()
{
  ...
  if (_standardize_params)
    _dgp.standardizeParameters(_training_params);
  // if not standardizing data set mean=0, std=1 for use in surrogate
  else
    _dgp.paramStandardizer().set(0, 1, _n_dims);

  // Standardize (center and scale) training data
  if (_standardize_data)
    _dgp.standardizeData(_training_data);
  // if not standardizing data set mean=0, std=1 for use in surrogate
  else
    _dgp.dataStandardizer().set(0, 1, _n_outputs);

  // Setup the covariance
  _dgp.setupCovarianceMatrix(_training_params, _training_data, _optimization_opts);
}
